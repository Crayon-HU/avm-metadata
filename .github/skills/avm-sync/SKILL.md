...
- Parse `.config/*.yaml` files (skip `modules.yaml`) — collect all `name:` values (full repo names, e.g. `terraform-azurerm-avm-res-network-virtualnetwork`)
...
3. Do **not** add `workspaces:` — this field no longer exists
...
- Do not modify `.config/modules.yaml` — this is a generated file
